def hangman():
    ...
    if(n<=0):
        ans.configure(text='ouch player, You just Loss The game......')
        res = messagebox.askyesno("Notification", 'ouch player, You just Loss The game......\n want to play again ?')
        if (res == True):
            chooseword()
        else:
            root.destroy()
...
#------------------------------------------------------------------------------------- Button
bt1 = Button(root,text='Submit',font=('arial',15,'bold'),width=15,bd=5,bg='red',activebackground='pink'
